...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	res, err := tx.Exec("INSERT INTO parcel (client, status, address, created_at) VALUES (:client, :status, :address, :created_at)",
...
func (s ParcelStore) SetStatus(number int, status string) error {
	...
	_, err = tx.Exec("UPDATE parcel SET status = :status WHERE number = :number",
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if status == ParcelStatusRegistered {
		tx, err := s.db.Begin()
		_, err = tx.Exec("UPDATE parcel SET address = :address WHERE number = :number",
